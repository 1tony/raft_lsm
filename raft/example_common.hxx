...
struct server_stuff {
    ...
    ptr<raft_server> raft_instance_;//实例化一个raft例程
};
static server_stuff stuff;//每个server都会维护一个静态量(相当于一个总接口，来调度其他不同功能的下层接口)
                ...
                const std::vector<std::string>& tokens)
{
    ...
    int server_id_to_add = atoi(tokens[1].c_str());//获取id号
    if ( !server_id_to_add || server_id_to_add == stuff.server_id_ ) {
        std::cout << "wrong server id: " << server_id_to_add << std::endl;
        return;
    }

    std::string endpoint_to_add = tokens[2];//获取地址号
    srv_config srv_conf_to_add( server_id_to_add, endpoint_to_add );//设定要给配置文件结构
                 ...
                 const std::vector<std::string>& tokens)
{
    ...
    stuff.raft_instance_->get_srv_config_all(configs);//configs应该当前服务器的配置文件
...
void loop() {
    char cmd[1000];
    std::string prompt = "kv" + std::to_string(stuff.server_id_) + "> ";
    while (true) {
